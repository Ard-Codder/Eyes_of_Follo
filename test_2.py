...
def resizing(new_width=None, new_height=None, interp=cv2.INTER_LINEAR):
    ...
    cv2.imwrite('trash_images/new_test_resize.jpg', res_img)

    # res_img = cv2.resize(img, (500, 900), cv2.INTER_NEAREST)
    # cv2.imwrite('res_  .jpg', res_img)
    ...
    # cv2.imwrite('  _nearest.jpg', res_img_nearest)
    # cv2.imwrite('  _linear.jpg', res_img_linear)
...
def shifting():
    ...
    cv2.imwrite('trash_images/test_right_and_down_1.jpg', dst)


def cropping():
    crop_img = img[10:450, 300:750]
    cv2.imwrite('trash_images/crop_face.png', crop_img)
...
def rotation():
    ...
    cv2.imwrite('trash_images/rotated_test.jpg', rotated)


resizing(1000,1000,10)